...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 10 00:36:10 2018v2
Created on Fri Mar 16 01:53:05 2018v3
...
def monthly_HTML_data():
    
    conn, cur = (ConnectToSQL())

    #iteam_id    
    item_id:str = []
    tbl=("SELECT id FROM master_table WHERE (t_date BETWEEN DATE_SUB(DATE_SUB(CURDATE()"+
    ...
    #client    
    client:str = []
    tbl=("SELECT client FROM master_table WHERE (t_date BETWEEN DATE_SUB(DATE_SUB(CURDATE()"+
        ",INTERVAL (DAY(CURDATE())-1) DAY), INTERVAL 1 MONTH) AND DATE_SUB(CURDATE(),"+
        "INTERVAL (DAY(CURDATE())) DAY));")
    ...
    tables = cur.fetchall()
    for table_name in tables:
        client.append((table_name))
        
    client = map(list,client)
    client_flat_list = [item for sublist in client for item in sublist]



    #state    
    state:str = []
    tbl=("SELECT state FROM master_table WHERE (t_date BETWEEN DATE_SUB(DATE_SUB(CURDATE()"+
        ",INTERVAL (DAY(CURDATE())-1) DAY), INTERVAL 1 MONTH) AND DATE_SUB(CURDATE(),"+
        "INTERVAL (DAY(CURDATE())) DAY));")
    ...
    state = map(list,state)
    state_flat_list = [item for sublist in state for item in sublist]

    #date    
    t_date:str = []
    tbl=("SELECT t_date FROM master_table WHERE (t_date BETWEEN DATE_SUB(DATE_SUB(CURDATE()"+
        ",INTERVAL (DAY(CURDATE())-1) DAY), INTERVAL 1 MONTH) AND DATE_SUB(CURDATE(),"+
        "INTERVAL (DAY(CURDATE())) DAY));")
    ...
    tables = cur.fetchall()
    for table_name in tables:
        t_date.append((table_name))
        
    t_date = map(list,t_date)
    t_date_flat_list = [item for sublist in t_date for item in sublist]

    #income    
    income:str = []
    tbl=("SELECT income FROM master_table WHERE (t_date BETWEEN DATE_SUB(DATE_SUB(CURDATE()"+
        ",INTERVAL (DAY(CURDATE())-1) DAY), INTERVAL 1 MONTH) AND DATE_SUB(CURDATE(),"+
        "INTERVAL (DAY(CURDATE())) DAY));")
    ...
    tables = cur.fetchall()
    for table_name in tables:        
        income.append((table_name))
        
    income = map(list,income)
    income_flat_list = [item for sublist in income for item in sublist]


    #expense    
    expense:str = []
    tbl=("SELECT expense FROM master_table WHERE (t_date BETWEEN DATE_SUB(DATE_SUB(CURDATE()"+
        ",INTERVAL (DAY(CURDATE())-1) DAY), INTERVAL 1 MONTH) AND DATE_SUB(CURDATE(),"+
        "INTERVAL (DAY(CURDATE())) DAY));")
    ...
    tables = cur.fetchall()
    for table_name in tables:        
        expense.append((table_name))
        
    expense = map(list,expense)
    expense_flat_list = [item for sublist in expense for item in sublist]

    #tax    
    tax:str = []
    tbl=("SELECT tax FROM master_table WHERE (t_date BETWEEN DATE_SUB(DATE_SUB(CURDATE()"+
        ",INTERVAL (DAY(CURDATE())-1) DAY), INTERVAL 1 MONTH) AND DATE_SUB(CURDATE(),"+
        "INTERVAL (DAY(CURDATE())) DAY));")
    ...
    tables = cur.fetchall()
    for table_name in tables:        
        tax.append((table_name))
        
    tax = map(list,tax)
    tax_flat_list = [item for sublist in tax for item in sublist]
...
#    return(len(item_id_flat_list),len(client_flat_list),len(state_flat_list),len(t_date_flat_list),len(income_flat_list),len(expense_flat_list),len(tax_flat_list))
    return((item_id_flat_list),(client_flat_list),(state_flat_list),(t_date_flat_list),(income_flat_list),(expense_flat_list),(tax_flat_list))

print(monthly_HTML_data())
